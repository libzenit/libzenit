...
void zenit_logger_trace(zenit_logger_t *logger, const char *fmt, ...) {
    ...
    va_end(args);
    zenit_logger_log(logger, ZENIT_LOG_TRACE, "%s", buf);
}
...
void zenit_logger_debug(zenit_logger_t *logger, const char *fmt, ...) {
    ...
    vsnprintf(buf, sizeof(buf), fmt, args);
    va_end(args);
    zenit_logger_log(logger, ZENIT_LOG_DEBUG, "%s", buf);
}
...
void zenit_logger_info(zenit_logger_t *logger, const char *fmt, ...) {
    ...
    vsnprintf(buf, sizeof(buf), fmt, args);
    va_end(args);
    zenit_logger_log(logger, ZENIT_LOG_INFO, "%s", buf);
}
...
void zenit_logger_warn(zenit_logger_t *logger, const char *fmt, ...) {
    ...
    vsnprintf(buf, sizeof(buf), fmt, args);
    va_end(args);
    zenit_logger_log(logger, ZENIT_LOG_WARN, "%s", buf);
}
...
void zenit_logger_error(zenit_logger_t *logger, const char *fmt, ...) {
    ...
    vsnprintf(buf, sizeof(buf), fmt, args);
    va_end(args);
    zenit_logger_log(logger, ZENIT_LOG_ERROR, "%s", buf);
}
...
void zenit_logger_fatal(zenit_logger_t *logger, const char *fmt, ...) {
    ...
    vsnprintf(buf, sizeof(buf), fmt, args);
    va_end(args);
    zenit_logger_log(logger, ZENIT_LOG_FATAL, "%s", buf);
    /* zenit_logger_log calls abort() for FATAL — unreachable */
}
