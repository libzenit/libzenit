...
static struct list_node *node_alloc(size_t elem_size, const void *elem) {
    struct list_node *n = malloc(sizeof(struct list_node) + elem_size);
    if (n == NULL) {
        return NULL;
    }
    n->prev = NULL;
...
zenit_list_t *zenit_list_create(size_t elem_size) {
    ...
    zenit_list_t *list = malloc(sizeof(zenit_list_t));
    if (list == NULL) {
        return NULL;
    }
...
zenit_result_t zenit_list_push_front(zenit_list_t *list, const void *elem) {
    ...
    struct list_node *n = node_alloc(list->elem_size, elem);
    if (n == NULL) {
