...
static void bench_encode_fn(void *ctx) {
    (void)ctx;
    char *e = zenit_hex_encode(test_data, 256);
    if (e) free(e);
}
...
static void bench_decode_fn(void *ctx) {
    (void)ctx;
    size_t len;
    unsigned char *d = zenit_hex_decode(encoded, &len);
    if (d) free(d);
}

int main(void) {
    for (int i = 0; i < 256; i++) test_data[i] = (unsigned char)i;
    encoded = zenit_hex_encode(test_data, 256);

    /* Encode */
    {
        zenit_bench_result_t r = zenit_bench_run(
            "hex_encode_256B", bench_encode_fn, NULL, 100000
        );
        zenit_bench_print(&r);
    }
    ...
    if (encoded != NULL) {
        zenit_bench_result_t r = zenit_bench_run(
            "hex_decode_256B", bench_decode_fn, NULL, 100000
        );
        zenit_bench_print(&r);
    }
